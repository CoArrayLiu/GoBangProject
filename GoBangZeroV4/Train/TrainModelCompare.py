...
class SelfCompete:
    def __init__(self):
        ...
        self.playerNet = PolicyValueNet((TCF.shape_first_dimension,self.board_size, self.board_size))
        ...
        self.evaluateGames = 2
        #在evaluateGames对局中，胜率大于等于0.6更新playerNet
        self.updateModelRate = 1.1
        #记录自我对弈的数据进行训练
        self.playDataPool = []
    ...
    def copyNet(self, Net):
        newNet = PolicyValueNet((TCF.shape_first_dimension,self.board_size, self.board_size))
...
class Train:
    ...
    def __init__(self):
        self.Net = PolicyValueNet((TCF.shape_first_dimension,self.board_size, self.board_size))
...
class Train_of_data:
    ...
    def train_of_extended_batch(self,fileIndex):
        ...
        print(f"本次平均loss:{sum(Loss)/len(Loss)}")
...
if __name__ == '__main__':
    ...
    # trainer.Net.load_model("../model/15/102.model")
    # for index in range(103,366,1):
    #     trainer.train_of_extended_batch(index)

    SC = SelfCompete()
    SC.playerNet.load_model("../model/15/169.model")
    SC.targetNet = PolicyValueNet((TCF.shape_first_dimension,SC.board_size,SC.board_size))
    SC.targetNet.load_model("../model/15/218.model")
