...
class ResidualCNN():
    ...
    def __init__(self, input_dim, output_dim, hidden_layers={"filters": 64, "kernel_size": (3, 3)}, num_layers=10):
    ...
    def ResLayer(self, input_block, filters, kernel_size):
        ...
        # 如果输入和输出的通道数不一致，则添加一个1x1卷积层来调整输入的通道数
        if K.int_shape(input_block)[1] != filters:
            input_block = Conv2D(filters=filters, kernel_size=(1, 1), data_format="channels_first", padding='same')(
                input_block)
    ...
    def BuildModel(self):
        ...
        conv = self.ConvLayer(input, 64, self.kernel_size)

        # 添加额外的卷积层
        conv = self.ConvLayer(conv, 128, self.kernel_size)  # 第一层额外卷积层
        conv = self.ConvLayer(conv, 256, self.kernel_size)  # 第二层额外卷积层
...
class PolicyValueNet(ResidualCNN):
    ...
    trainDataPoolSize = config.trainDataPoolSize  # 用以存储训练网络的数据
    trainBatchSize = config.trainBatchSize  # 每次从数据池(trainDataPool)中随机采样出的一批训练数据
    epochs = config.epochs  # 每次训练步数

    trainDataPool = deque(maxlen=trainDataPoolSize)  # 训练数据池

    kl_targ = 0.02
    learningRate = config.learning_rate # 学习率
    ...
    def policy_NN(self, input):
        ...
        """
        if not config.UseDropArea:
            emptySpace = input.availables  # 棋盘目前的空位(即可以落子的地方)
        else:
            emptySpace = input.drop_area   # 已落子区域周围Border格
        # if len(emptySpace) == 0:
        #     emptySpace = input.availables
    ...
    def train(self, batchBoard, batchProbs, batchWinner, learning_rate):
        ...
        # 评价当前模型
        loss = self.model.evaluate(oneBatchBoard, [oneBatchProbs, oneBatchWinner], batch_size=len(batchBoard),
                                   verbose=0)
        # 重新设置模型的学习率
    ...
    def update_net_extended(self,train_set):
        """
        更新预测网络的参数
        """
        # trainBatchSize 三元组
        # 棋盘状态 + 落子概率分布 + 胜者预测
        batchBoard = [data[0] for data in train_set]
        batchProbs = [data[1] for data in train_set]
        batchWinner = [data[2] for data in train_set]
    ...
    def update_of_board(self,train_set):
        """
        更新预测网络的参数
        """
        # 从数据池中随即采样一批数据进行训练
        trainBatchSize = train_set
        #print(train_set)
        # trainBatchSize 三元组
        # 棋盘状态 + 落子概率分布 + 胜者预测
        batchBoard = train_set[0]
        batchProbs = train_set[1]
        batchWinner = train_set[2]

        # 返回这一批训练数据样本的模型预测值
        # 也就是首先记录下当前模型的预测水平
        print(np.array(batchBoard).shape)
        batchProbsOld, batchValueOld = self.model.predict_on_batch(np.array(batchBoard))
