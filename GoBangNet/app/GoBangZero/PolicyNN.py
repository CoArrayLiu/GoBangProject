...
class PolicyValueNet(ResidualCNN):
    ...
    trainDataPoolSize = GBC.trainDataPoolSize  # 用以存储训练网络的数据
    trainBatchSize = GBC.trainBatchSize  # 每次从数据池(trainDataPool)中随机采样出的一批训练数据
    epochs = GBC.epochs  # 每次训练步数

    trainDataPool = deque(maxlen=trainDataPoolSize)  # 训练数据池

    kl_targ = 0.02
    learningRate = GBC.learning_rate
