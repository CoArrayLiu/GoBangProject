...
class MCTS():
    ...
    def get_bonus_prob(self, pos,len):
        if 0<=pos and pos<=4:
            return 0.1/len
        if 10<=pos and pos<=14:
            return 0.1/len
    def border_explorer_balance_element(self,action,len):
        x = GBC.BOARD_SIZE - action // GBC.BOARD_SIZE - 1
        y = action % GBC.BOARD_SIZE
        x, y = y, x
        bonus = self.get_bonus_prob(x,len) + self.get_bonus_prob(y,len)
        return bonus
    ...
    def getMoveProbs(self, state, flag_is_train):
        ...
        exploration = TCF.train_exploration if flag_is_train else 1e-3
        ...
        # if TCF.UseBorderStrengthen:
        #     availables = len(acts)
        #     for index, act in enumerate(acts):
        #         bonus = self.border_explorer_balance_element(act,availables)
        #         act_probs[index] += bonus
        #     sum_prob = np.sum(act_probs)
        #     act_probs = act_probs / sum_prob
